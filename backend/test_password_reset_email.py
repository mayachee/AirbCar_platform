...
Run: docker-compose exec web python /app/test_password_reset_email.py
"""
import os
import django
...
if not user:
    print(f"❌ ERROR: User with email {test_email} not found in database!")
    print("\nAvailable users:")
...
# Create email
subject = 'TEST - Password Reset Request - AirbCar'
text_content = f'''
Hello,
...
<body>
    <div class="container">
        <h2>TEST - Password Reset Request</h2>
        <p>Hello,</p>
        <p>You requested to reset your password for your AirbCar account.</p>
        <p>Click the button below to reset your password:</p>
        <a href="{reset_url}" class="button">Reset Password</a>
        <p>Or copy and paste this link into your browser:</p>
        <p style="word-break: break-all; color: #666; font-family: monospace;">{reset_url}</p>
...
try:
    ...
    print()
    print("🔍 CHECK THESE PLACES IN GMAIL:")
    print("  1. Inbox")
    print("  2. Spam/Junk folder")
    print("  3. Promotions tab")
    print("  4. Social tab")
    print("  5. Updates tab")
    print("  6. All Mail (search for 'AirbCar' or 'Password Reset')")
    print()
    print("🔎 GMAIL SEARCH COMMANDS:")
    print(f"  from:{settings.EMAIL_HOST_USER}")
    print(f"  subject:\"Password Reset\"")
    print(f"  \"AirbCar\"")
    print(f"  \"TEST - Password Reset\"")
    print()
    print("⏰ Note: Gmail may take 1-2 minutes to deliver the email.")
    print("   If you don't see it, wait a few minutes and check again.")
    print()
    print(f"🔗 Reset URL (if email doesn't arrive):")
    print(f"   {reset_url}")
    print()
